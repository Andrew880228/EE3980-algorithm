...
int main(void)
{
    ...
    // selling day should shift to the day before
    ...
    for(i = 0; i < R; i++)              // repeat 1000 times
    {
...
RSLT MaxSubArrayBF()                        // brute-force solution
{
    int i, j, k;                            // for loops
    RSLT result;                            // for storing low, high, change
    result.max = 0.0;                       // initializations of result
    result.sell = 0;
    result.buy = N-1;
    ...
    for (i = 0; i < N; i++)                 // determine start index
    {
        for (j = i; j < N; j++)             // determine end index
        {
            ...
            if (sum < result.max)
            {
                result.max = sum;           // found max, update informations
                result.sell = i;
                result.buy = j;
            }
        }
...
RSLT MaxSubArray(int begin, int end)        // divide and conquer approach
{
    ...
    int mid = (begin + end) / 2;            // determine middle index
    ...
    if (lsum.max <= rsum.max && lsum.max <= xsum.max)
    {
        return lsum;
    }
    else if (rsum.max <= lsum.max && rsum.max <= xsum.max)
    {
        return rsum;
    }
    return xsum;
...
RSLT MaxSubArrayXB(int begin, int mid, int end) // check cross boundary condition
{
    RSLT result;                            // store information
    int i;                                  // for loop
    double sum;                             // for checking sum
    
    double lsum = 0.0;                      // left hand side's sum
    ...
    for (i = mid; i > begin; i--)
    {
        ...
        if (sum < lsum)                     // determine left max
        {
    ...
    double rsum = 0.0;                      // right hand side's sum
    ...
    result.max = rsum + lsum;               // update total sum
    return result;
}
